...
import { sendPushNotification as sendBuyerPush } 
...
export const buyerResolvers = {
  ...
  Mutation: {
    sendBuyerWelcomeNotification: async (_, { firebaseUid }) => {
  try {
    if (!firebaseUid) {
    ...
    // 🔒 Prevent duplicates
    if (buyer.welcomeNotificationSent) {
      return {
        success: true,
    ...
    buyer.welcomeNotificationSent = true;
    await buyer.save();
...
signupBuyer: async (
  ...
  try {
    ...
    if (!firebaseUser.email?.toLowerCase().endsWith("@gmail.com")) {
      throw new Error("Only Google email addresses are allowed");
    }
    ...
    if (!buyer.buyerId) {
      throw new Error("buyerId generation failed");
    }

    // 5️⃣ Resolve Firebase UID mapping
    await resolveFirebaseUid(buyer);

    // 6️⃣ ADMIN NOTIFICATION (NON-BLOCKING)
    try {
    ...
    // 7️⃣ BUYER PUSH NOTIFICATION (NON-BLOCKING)
    try {
      ...
      console.warn("⚠ Buyer push failed:", pushErr.message);
    }
...
resendBuyerEmailVerification: async (_, { firebaseUid }) => {
  try {
    if (!firebaseUid) {
      throw new Error("Firebase UID is required");
    }
    ...
    // 🔐 Generate secure Firebase verification link
    const verificationLink =
      await auth.generateEmailVerificationLink(user.email);

    // 📧 Send link via your mail service
...
removeBuyerFcmToken: async (_, { buyerId, token }) => {
  const buyer = await Buyer.findOne({ buyerId });
  if (!buyer) throw new Error("Buyer not found");

  buyer.fcmTokens = buyer.fcmTokens.filter(t => t !== token);
  buyer.fcmTokenMeta = buyer.fcmTokenMeta.filter(m => m.token !== token);
