  ...
  Mutation: {
    ...
    updateOrderStatus: async (
      ...
      const messageMap = {
        ...
        shipped: `Your order ${orderId} has been shipped.${order.trackingNumber ? ` Tracking No: ${order.trackingNumber}` : ""
          }`,
      ...
      if (status === ORDER_STATUS.SHIPPED) {
        await sendWhatsappMessage(
          buyer.phone,
          `🚚 Your Flyhub order ${orderId} has been shipped.${order.trackingNumber ? ` Tracking No: ${order.trackingNumber}` : ""
          }`
        );
      }
